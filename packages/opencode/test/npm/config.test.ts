...
describe("NpmConfig", () => {
  ...
  test("merges config relative to a directory with env last", async () => {
    ...
    try {
      const config = await NpmConfig.flat(dir)
      expect(config.registry).toBe("https://env.example/")
      expect(config.ignoreScripts).toBe(false)
      expect(config.binLinks).toBe(false)
      expect(config["@scope:registry"]).toBe("https://scope.example/")
    } finally {
      restore()
    }
  })

  test("preserves hard safety defaults for arborist", async () => {
    await using tmp = await tmpdir()
    const global = path.join(tmp.path, "global.npmrc")
    const user = path.join(tmp.path, "user.npmrc")
    ...
    await Bun.write(global, "registry=https://global.example/\n")
    await Bun.write(user, "registry=https://user.example/\n")
    await Bun.write(local, "bin-links=false\nignore-scripts=false\nprogress=true\nsave-prefix=^\n")
    await Bun.write(pkg, '{"name":"repo","version":"1.0.0"}\n')
    ...
    try {
      const options = await NpmConfig.arboristOptions(dir)
      expect(options.path).toBe(dir)
      expect(options.binLinks).toBe(true)
      expect(options.ignoreScripts).toBe(true)
      expect(options.progress).toBe(false)
      expect(options.savePrefix).toBe("")
      expect(options.registry).toBe("https://user.example/")
    } finally {
      restore()
    }
  })
